...
class evs_page extends StatefulWidget {
  const evs_page({Key? key}) : super(key: key);

  @override
  State<evs_page> createState() => _evs_pageState();
}

class _evs_pageState extends State<evs_page> {
  List evs_page = [];
  ...
  _initData() async {
    await DefaultAssetBundle.of(context)
        .loadString("assets/Evs.json")
        .then(((value) {
      setState(() {
        evs_page = json.decode(value);
      });
    }));
  }
  ...
  _initializeVideo(int index) async {
    final controller = VideoPlayerController.networkUrl(
        Uri.parse(evs_page[index]["videoUrl"]));
                  ...
                  Container(
                    ...
                    decoration: BoxDecoration(
                        ...
                        image: DecorationImage(
                          image: AssetImage(evs_page[index]["thumbnail"]),
                          fit: BoxFit.cover,
                        )),
                  ),
                  SizedBox(width: 16),
